...
# Schema reference:
# Birth_Registration:    birth_id, citizen_id, father_id, mother_id, hospital_id,
#                        birth_certificate_number, registration_date
# Death_Registration:    death_id, citizen_id, date_of_death, cause_of_death,
#                        place_of_death, death_certificate_number, registration_date
# Marriage_Registration: marriage_id, husband_id, wife_id, marriage_date,
#                        registrar_officer_id, marriage_certificate_number, registration_date
...
def register_birth():
    ...
    ok, err = require_fields(data, "citizen_id", "registration_date")
    if not ok:
    ...
    def ops(conn, cursor):
        cert_number = f"BC-{data['citizen_id']}-{data['registration_date'].replace('-','')}"
        cursor.execute(
            """INSERT INTO Birth_Registration
               (citizen_id, father_id, mother_id, hospital_id,
                birth_certificate_number, registration_date)
               VALUES (%s, %s, %s, %s, %s, %s)""",
            (data["citizen_id"],
             data.get("father_id"),
        ...
        cursor.execute(
            """INSERT INTO Death_Registration
               (citizen_id, date_of_death, cause_of_death,
                place_of_death, death_certificate_number)
               VALUES (%s, %s, %s, %s, %s)""",
            (data["citizen_id"],
             data["date_of_death"],
...
def register_marriage():
    ...
    def ops(conn, cursor):
        ...
        for cid in [data["husband_id"], data["wife_id"]]:
            cursor.execute(
                "UPDATE Citizen SET marital_status = 'Married' WHERE citizen_id = %s", (cid,))
